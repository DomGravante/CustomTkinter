...
class CTkButton(CTkBaseClass):
    ...
    def draw(self, no_color_updates=False):
        ...
        self.content = tkinter.Frame(master=self)
        self.content.bind("<Enter>", self.on_enter)
        self.content.bind("<Leave>", self.on_leave)
        self.content.bind("<Button-1>", self.clicked)
        self.content.bind("<Button-1>", self.clicked)
        ...
        if self.image is not None:
            ...
            if no_color_updates is False:
                ...
                self.content.configure(bg=ThemeManager.single_color(self.fg_color, self._appearance_mode))
        ...
        sticky = self.anchor if self.anchor else "ew"
        #self.content.grid(row=0,column=0, rowspan=2, columnspan=2, sticky=sticky,
        #             padx=(max(self.apply_widget_scaling(self.corner_radius), self.apply_widget_scaling(self.border_width)),
    ...
    def on_enter(self, event=0):
        if self.hover is True and self.state == tkinter.NORMAL:
            ...
            self.content.configure(bg=ThemeManager.single_color(inner_parts_color, self._appearance_mode))
